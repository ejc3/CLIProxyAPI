...
func TestBackendQuotaExhaustedRequiresTypedEvidence(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if got := backendQuotaExhausted(tc.message); got != tc.want {
				t.Fatalf("quota=%t want=%t", got, tc.want)
			}
		})
	}
}
...
func fallbackTestChain(t *testing.T, executors ...coreauth.ProviderExecutor) *FallbackBackend {
	...
	for index, executor := range executors {
		...
		if _, err := manager.Register(t.Context(), &coreauth.Auth{ID: opts.AuthID, Provider: opts.Provider, Status: coreauth.StatusActive}); err != nil {
			t.Fatal(err)
		}
	...
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("%s-%t", tc.path, tc.stream), func(t *testing.T) {
			...
			for request := 0; request < 2; request++ {
				...
				if w.Code != 200 || strings.Contains(w.Body.String(), "PRIVATE-") || w.Header().Get("Retry-After") != "" {
					t.Fatalf("failed recovery: %d %s headers=%v", w.Code, w.Body.String(), w.Header())
				}
			}
			if first.calls != 1 || second.calls != 1 || third.calls != 2 {
				t.Fatalf("wrong attempts: %d %d %d", first.calls, second.calls, third.calls)
			}
			if !reflect.DeepEqual(transitions, [][2]int{{0, 1}, {1, 2}}) {
				t.Fatalf("wrong transitions: %v", transitions)
			}
			if observation.result() != BackendErrorNone {
				t.Fatal("recovered error tainted final diagnostics")
			}
			for index, capture := range []*backendCapture{first, second, third} {
				if capture.authID != fmt.Sprintf("%s-profile-%d", t.Name(), index) || capture.opts.Metadata[coreexecutor.PinnedAuthMetadataKey] != capture.authID {
					t.Fatal("fallback escaped pinned credential")
				}
				if !strings.Contains(string(capture.req.Payload), "preserve this prompt") || strings.Contains(string(capture.req.Payload), "PRIVATE-") || capture.opts.Headers.Get("Authorization") != "" {
					t.Fatal("fallback damaged content or leaked master identity")
				}
				if !reflect.DeepEqual(capture.opts.Headers.Values("Anthropic-Beta"), nativeProtocolFixture().Values("Anthropic-Beta")) {
					t.Fatal("native protocol headers changed during fallback")
				}
			}
		})
	}
...
func TestFallbackBackendDoesNotRotateOnOtherFailures(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if first.calls != 1 || second.calls != 0 || w.Code < 400 || strings.Contains(w.Body.String(), "PRIVATE-") {
				t.Fatalf("unexpected fallback: calls=%d/%d status=%d", first.calls, second.calls, w.Code)
			}
			if index, _ := b.current(); index != 0 {
				t.Fatal("non-quota failure advanced the chain")
			}
		})
	}
}
...
func TestFallbackBackendRemembersAllExhausted(t *testing.T) {
	...
	for request := 0; request < 3; request++ {
		w := httptest.NewRecorder()
		b.ServeHTTP(w, fallbackTestRequest("/v1/messages", false))
		if w.Code != 429 || !strings.Contains(w.Body.String(), "All configured inference profiles") || w.Header().Get("Retry-After") != "3600" || w.Header().Get("Set-Cookie") != "" || strings.Contains(w.Body.String(), "PRIVATE-") {
			t.Fatalf("bad exhausted response: %d %s %v", w.Code, w.Body.String(), w.Header())
		}
	}
	if first.calls != 1 || second.calls != 1 {
		t.Fatal("exhausted account retried after SDK cooldown")
	}
}
...
func (e *fallbackLateQuotaExecutor) ExecuteStream(ctx context.Context, auth *coreauth.Auth, request coreexecutor.Request, options coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	...
	go func() {
		...
		select {
		case chunks <- coreexecutor.StreamChunk{Err: fallbackTestQuota()}:
		case <-ctx.Done():
		}
	}()
...
func TestFallbackBackendNeverReplaysCommittedStream(t *testing.T) {
	...
	if first.calls != 1 || second.calls != 0 || !strings.Contains(w.Body.String(), "already-generated") || !strings.Contains(w.Body.String(), "event: error") {
		t.Fatalf("committed stream replayed or lost: %d/%d %s", first.calls, second.calls, w.Body.String())
	}
	if index, _ := b.current(); index != 1 {
		t.Fatal("confirmed late quota did not advance future requests")
	}
	next := httptest.NewRecorder()
	b.ServeHTTP(next, fallbackTestRequest("/v1/messages", false))
	if next.Code != 200 || second.calls != 1 {
		t.Fatal("next request did not use fallback after late quota")
	}
}
	...
	for _, kind := range []string{"flush", "header", "failed-write"} {
		t.Run(kind, func(t *testing.T) {
			...
			if kind == "failed-write" {
				underlying = &fallbackFailedWriter{header: make(http.Header)}
			}
			...
			if !w.committed {
				t.Fatal("response boundary was not committed")
			}
		})
	}
}
...
func TestFallbackBackendConcurrentExhaustionTransitionsOnlyOnce(t *testing.T) {
	...
	b.SetOnFallback(func(from, to int) {
		if from != 0 || to != 1 {
			t.Error("incorrect concurrent transition")
		}
		notices.Add(1)
	})
	...
	for i := 0; i < requests; i++ {
			...
			if w.Code != 200 || w.Body.String() != "success" {
				t.Errorf("concurrent recovery failed: %d %s", w.Code, w.Body.String())
			}
		})
	}
	for i := 0; i < requests; i++ {
	...
	if notices.Load() != 1 || next.Load() != requests {
		t.Fatalf("transitions=%d recovered=%d", notices.Load(), next.Load())
	}
}
...
func TestNewFallbackBackendPreflightsEveryProfile(t *testing.T) {
	...
	// NewBackend validates against the pinned catalog; the helper's default model is not in it.
	first.Model, second.Model = "gpt-5.6-sol", "gpt-5.6-sol"
	for _, tc := range []struct {
		name    string
		...
		t.Run(tc.name, func(t *testing.T) {
			if backend, err := NewFallbackBackend(t.Context(), tc.options); err == nil {
				_ = backend.Close()
	...
	b.ServeHTTP(w, fallbackTestRequest("/v1/messages", false))
	if w.Code != 503 {
		t.Fatal("closed chain accepted inference")
	}
}
