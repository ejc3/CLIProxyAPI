...
func TestBackendRealClaudeExecutor(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, header := range []string{"Authorization", "X-Api-Key", "Cookie", "Proxy-Authorization", "X-Account-Id", "User-Agent", "X-Claude-Code-Session-Id"} {
			...
			if tc.stream && (!strings.HasPrefix(response.Header().Get("Content-Type"), "text/event-stream") || !strings.Contains(response.Body.String(), "message_stop")) {
