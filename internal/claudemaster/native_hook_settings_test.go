...
func TestNativeHookSettingsImmutableSnapshot(t *testing.T) {
	...
	if err := os.WriteFile(file, []byte(hookSettingsFixture), 0o600); err != nil {
		t.Fatal(err)
	}
	for _, args := range [][]string{{"--settings", file, "--remote-control"}, {"--settings=" + file}, {"--settings", hookSettingsFixture}} {
		...
		if strings.Contains(strings.Join(got, " "), file) {
			t.Fatal("mutable file passed to native")
		}
		if _, err := ChildEnvironment(nil, got, "proxy", "ca"); err != nil {
			t.Fatal(err)
		}
		if !strings.Contains(strings.Join(got, " "), "session-sync.sh") {
			t.Fatal("session tracking hook lost")
		}
	}
	...
	if err := os.WriteFile(file, []byte(`{"env":{"NO_PROXY":"*"}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := ChildEnvironment(nil, got, "proxy", "ca"); err != nil {
		t.Fatal("snapshot changed with source", err)
	}
	if _, err := ChildEnvironment(nil, []string{"--settings", file}, "proxy", "ca"); err == nil {
		t.Fatal("environment boundary accepted mutable file")
	}
}
...
func TestNativeHookSettingsRejectUnsafeSourcesAndOverrides(t *testing.T) {
	...
	for _, args := range [][]string{{"--settings"}, {"--settings="}, {"--settings", "{}", "--settings={}"}} {
		if _, err := snapshotNativeHookSettings(args); err == nil {
			t.Fatal("invalid settings arguments accepted")
		}
	}
	for _, kind := range []string{"symlink", "fifo", "directory", "oversized"} {
		t.Run(kind, func(t *testing.T) {
			...
			case "fifo":
				err = unix.Mkfifo(file, 0o600)
			case "directory":
				err = os.Mkdir(file, 0o700)
			case "oversized":
				err = os.WriteFile(file, []byte(strings.Repeat(" ", hookSettingsByteLimit+1)), 0o600)
			}
			if err != nil {
				t.Fatal(err)
			}
			if _, err := snapshotNativeHookSettings([]string{"--settings", file}); err == nil {
...
func TestNativeHookSettingsRespectsArgumentTerminator(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(got, args) {
		t.Fatalf("prompt changed: %v %v", got, err)
	}
	if _, err := ChildEnvironment(nil, got, "proxy", "ca"); err != nil {
		t.Fatal(err)
	}
