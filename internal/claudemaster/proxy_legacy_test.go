...
func TestProxyLegacyRemoteControlBoundary(t *testing.T) {
	...
	_, client, _ := proxyTestStart(t, http.HandlerFunc(func(http.ResponseWriter, *http.Request) { t.Error("legacy control reached inference") }), proxyTestTransport(func(r *http.Request) (*http.Response, error) {
		calls++
		if r.Header.Get("Authorization") != "Bearer master-control-only" { t.Error("legacy master authorization changed") }
		if r.Header.Get("X-Environment-Runner-Version") == "" && r.Header.Get("X-Organization-Uuid") != "org_native" { t.Error("legacy CLI control identity changed") }
		if r.URL.Path == "/v1/sessions" {
			body, err := io.ReadAll(r.Body)
			if err != nil || (string(body) != creation && string(body) != strings.Replace(creation, `"environment_id":"env_native"`, `"self_hosted_runner_pool_id":"ccpool_native"`, 1)) { t.Error("validated native creation body changed") }
		}
	...
	for _, tc := range []struct{ method, path, body string }{
		...
		if resp.StatusCode != 204 { t.Errorf("native %s %s was blocked: %d", tc.method, tc.path, resp.StatusCode) }
	}
	if calls != 7 { t.Fatalf("expected seven native control dispatches, got %d", calls) }
	for _, action := range []string{"events", "archive"} {
		...
		if resp.StatusCode != 204 { t.Errorf("native bridge %s was blocked: %d", action, resp.StatusCode) }
	}
	for _, tc := range []struct{ method, path, body, beta string }{
		...
		h := headers.Clone(); h.Set("Anthropic-Beta", tc.beta)
		resp, _ := proxyTestRequest(t, client, tc.method, tc.path, tc.body, h)
		if resp.StatusCode != 403 { t.Errorf("non-native %s %s escaped legacy boundary: %d", tc.method, tc.path, resp.StatusCode) }
	}
	bridge := headers.Clone()
	bridge.Del("X-Organization-Uuid")
	bridge.Set("Anthropic-Beta", "environments-2025-11-01")
	...
	for _, path := range []string{"/v1/sessions", "/v1/sessions/session_native/unarchive"} {
		...
		if resp.StatusCode != 403 { t.Errorf("bridge profile escaped its callback paths: %s", path) }
	}
	if calls != 9 { t.Fatal("rejected legacy request reached master control") }
}
